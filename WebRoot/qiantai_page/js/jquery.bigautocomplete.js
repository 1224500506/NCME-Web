(function($){
	var bigAutocomplete = new function(){
		...
		this.init = function(){
			...
			$("#bigAutocompleteContent").delegate("tr", "click", function() {
				...
				// 选中单元格时赋值
				$("#hospital_address").val($(this).find("div:last").attr("id"));
				ChangeData();
		...
		this.autocomplete = function(param){
			...
			$this.keydown(function(event) {
				...
				case 40://向下键
					...
					if($nextSiblingTr.length <= 0){//没有选中行时，选中第一行
						...
						$("#hospital_address").val($nextSiblingTr.find("div:last").attr("id"));
						ChangeData();
					}else{
				...
				case 38://向上键
					...
					if($previousSiblingTr.length <= 0){//没有选中行时，选中最后一行行
						...
						$("#hospital_address").val($previousSiblingTr.find("div:last").attr("id"));
						ChangeData();
			...
			function makeContAndShow(data_){
				if(data_ == null || data_.length <=0 ){
					//添加其它选项-----taol
					var cont = "<table style='margin: 0; padding:0;'><tbody>";
					cont += "<tr><td><div id ='其它'>其它</div></td></tr>"
					cont += "</tbody></table>";
				...
				for(var i=0;i<data_.length;i++){
					cont += "<tr><td><div id ="+data_[i].content+">" + data_[i].title + "</div></td></tr>"
				}
				cont += "<tr><td><div id ='其它'>其它</div></td></tr>"
				cont += "</tbody></table>";
				$bigAutocompleteContent.html(cont);
	...
	function ChangeData(){
		...
		if($("#hospital_address").val() == 0) {
			...
			$("#hst_lvl").show();
			$("#hospital_region").val("");
			var url = basepath+"/examManage/getHospitalLevelAjax.do?type=23";
			$.ajax({
			    url:url,
			    type: 'POST',
			    dataType: 'json',
		...
		}else if($("#hospital_address").val() > 0){
			$("#other_hospital_name").val("");
			$("#other_hospital_name").hide();
		}else {
			var adr = $("#hospital_address").val();
			if(adr == '其它'){
				$("#other_hospital_name").val("");
				$("#other_hospital_name").show();
			}else{
				$("#other_hospital_name").hide();
				$("#hst_lvl").hide();
				var url = basepath+"/examManage/getHospitalListAjax.do?parentId="+$("#rspropid").val();
				$.ajax({
				    url:url,
				    type: 'POST',
				    dataType: 'json',
