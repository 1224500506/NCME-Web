                           ...
                           <c:forEach items = "${list.unitList}" var="unit" varStatus="status">
	                            <li class="has_done">
	                             <i class="iconfont pm" id="pm">&#xe674;</i>    
	                                ...
	                                <a class="unit" title="${unit.id}" href="javascript:void(0);" onclick="zkContrent('${unit.id}',this,'${unit.content}','${unit.quota}');">单元${status.index+1}:${unit.name}</a><i class="fa fa-info-circle fa-info"> 任务点</i>
...
<script>
 ...
 // var adminURL = "http://localhost:8081/admin";
  ...
  //var peixunURL = "http://localhost:8081/test_peixun";
    ...
    //window.beforeunload = winLeave; //YHQ，2017-03-10
    ...
    $(function () {
        $(".go_back").click(function () {
            document.location.href = ctxJS + "/projectView.do?id=${project.id}";
        });
    ...
    function zkContrent(obj,dom,type,quota){
        winLeave();
        vLength   = 0 ; //YHQ，2017-03-09，视频长度（秒）
        vPosition = 0 ; //YHQ，2017-03-09，视频播放位置（秒）
        vFlag = false ; //YHQ，2017-03-09，是否视频
        videoCvsetId = 0;//YHQ，2017-03-09
        videoCvId = 0 ;//YHQ，2017-03-09
        videoUnitId = 0;//YHQ，2017-03-09
        videoLogStudyId = 0;//YHQ，2017-03-09
    	...
    	//设置单元图标
    	//$(dom).prev().html("class","fa fa-check-circle");
    	//$(dom).prev().html("&#xe61c;");
		    		...
		    		if(unit!=null && unit.length>0){
		    			...
		    			if(content!='' && content!=null){
		    				...
		    				if(alt!='bingli' && alt!='paper' && alt!='talk'){
		    					play($("#preview img"),${project.id},cvId,unitId,videoLogStudyId,$(dom).parent().parent().prev().attr("class"),$(dom).parent().parent().prev().text(),type,$(dom).text(),quota);
		    				}else{
    ...
    //点击播放    
    function play(obj,cvsetIdVal,cvIdVal,unitIdVal,logStudyIdVal,cvId,cvName,type,unitName,quota){
    ...
    function queryUnitTalks(unitId){
      ...
      */
      function preview(id,type,unitId){
    	 ...
    	 if(type=='paper'){
    		 var html = "<iframe src=\""+peixunURL+"/paperManage/paperView.do?handle=exam&paperId="+id+"&unitId="+unitId+"\" style=\"width:100%;height:100%;\"></iframe>"
    		 document.getElementById("preview").innerHTML = html; 
    	 }
    	 if(type=='talk'){
    ...
    /***程宏业2017-03-24 子页面操作完成后调用父页面的方法刷新状态
    **/ 
     function load(){
    	 tongjiUnitStudy('${project.id}');
     } 
