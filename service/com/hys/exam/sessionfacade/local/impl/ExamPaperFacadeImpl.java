...
public class ExamPaperFacadeImpl extends BaseSessionFacadeImpl implements
	...
	public Long addExamPaper(ExamPaper paper) throws FrameworkRuntimeException {
		...
		// 卷中卷
		if(paper.getPaper_mode() == Constants.PAPER_MODE_S){
	...
	public List<ExamPaper> getExamPaperList(ExamPaperQuery examPaperQuery)
		...
		try {
			// 查询试卷
			return localExamPaperManage.getExamPaperList(examPaperQuery);
		} catch (Exception e) {
			logger.error("",e);
			throw new FrameworkRuntimeException("查询试卷出现异常!");
		}
	...
	public void updateExamPaper(ExamPaper paper)
		...
		if (paper.getPaper_mode() == Constants.PAPER_MODE_RM) {
			...
			if (tacticList != null && (!tacticList.isEmpty())) {
				...
				getPaperQuestion(paper);
			}		
			// 取试卷和子试卷列表
			List<ExamPaper> paperList = localExamPaperManage.getExamPaperAndChildPaper(new Long[]{paper.getId()});
		...
		try {
			// 修改试卷信息
			localExamPaperManage.updateExamPaper(paper);
		} catch (Exception e) {
			logger.error("",e);
