...
//        String logImageName = multipartFile.getOriginalFilename();  
        ...
        /**拼成完整的文件保存路径加文件**/    
        String fileName = id + logImageName+suffix;
        String filePath="";
        if(id != null)
        	filePath = newLogoRelPathDir + "/" + fileName;
        else
        	filePath = newLogoRelPathDir + "/" + defaultName;
        
        try {     
        	FileOutputStream outer = new FileOutputStream(new File(filePath));   
            byte[] buffer = file.getFileData();   
            outer.write(buffer);   
            outer.close();   
            file.destroy();      
	...
	//杨红强把头像传到私有云中，YHQ 2017-02-20，改自：public static String fileUpload(FormFile file,HttpServletRequest request,String fileDir,Long id,String defaultName,String savePath)
    public static String fileUploadToES(FormFile file,HttpServletRequest request,Long fileNameId) throws Exception {          
        //无文件上传
        if (file == null || "".equals(file.getFileName()))
        	return null;  
        ...
        if ((jsonObject != null) && (!jsonObject.equals(""))) {
          if ((jsonObject.get("code").equals("200")) && (jsonObject.get("message").equals("successful"))) {
        	  ...
        	  sourceKey = sourceKey.substring(sourceKey.indexOf("KeyId=") + 6, sourceKey.length());
        	  sourceKey = sourceKey.substring(0, sourceKey.indexOf("&")) ;
        	  
        	  String imgUrl = ESUtil.getUpdateFileDownloadUrl(ESConfig.DEFAULT_FOLDER,ESConfig.DEFAULT_BUCKET, sourceKey) ;
