...
public class ExamQuestionJDBCDAO extends BaseDao implements ExamQuestionDAO {
	...
	*/
	...
	private static final String SQL_GET_QUESTIONLIST_BY_TACTIC =  "select (select count(*) from EXAM_QUESTION where parent_id = q.id) as childQuestionNum, q.* from EXAM_QUESTION q, exam_sys_quest_type_quest t where q.id = t.question_id and q.parent_id = 0 and q.state = 4 and t.state = 1 and q.question_label_id = ? ";
	...
	private static final String SQL_GET_QUESTION_SIZE_BY_FASTER_TACTIC = "select distinct q.id from EXAM_QUESTION q, exam_sys_quest_type_quest t where q.id = t.question_id and q.parent_id = 0 and q.state = 4 and t.state = 1 ";

	private static final String SQL_GET_QUESTION_LIST_BY_FASTER_TACTIC = "select distinct q.* from EXAM_QUESTION q, exam_sys_quest_type_quest t where q.id = t.question_id and q.parent_id = 0 and q.state = 4 and t.state = 1 ";
	...
	public ExamQuestion addQuestion(ExamQuestion quest,int saveParent) {
		...
		if(child!=null){
			...
			for(int i=0;i<child.size();i++){
				...
				childQuestion.setId(getNextId(Constants.TABLE_EXAM_QUESTION));
	...
	public void updateQuestionStateByIdArr(List<Long> id, int state,String author, String opinion) {
		List<Object[]> batch = new ArrayList<Object[]>();
		StringBuffer sql = new StringBuffer("update exam_question set create_date = sysdate(), state = ?");
		
		// 创建者
		if (!StringUtils.checkNull(author)) {
			sql.append(",checker = ?, opinion = ?");
			for (int i = 0; i < id.size(); i++) {
				Object[] values = new Object[] { state, author, opinion, id.get(i), id.get(i) };
				batch.add(values);
			}
		} else{
			for (int i = 0; i < id.size(); i++) {
		...
		sql.append(", online_date = sysdate()");
		sql.append(" where id = ? or parent_id = ?");
		
		getSimpleJdbcTemplate().batchUpdate(sql.toString(), batch);
	}
	...
	/*	List<ExamQuestionProp> questUnitProp = questQuery.getQuestionPropMap().get(Constants.EXAM_PROP_UNIT);
		List<ExamQuestionProp> questSource = questQuery.getQuestionPropMap().get(Constants.EXAM_PROP_SOURCE);
		List<ExamQuestionProp> questCognize = questQuery.getQuestionPropMap().get(Constants.EXAM_PROP_COGNIZE);
		
		if (questUnitProp != null) {
		
		for (int i=0; i<questUnitProp.size(); i++) {
			String my_sql = "select prop_val_id from SUB_SYS_PROP_VAL where id=" + questUnitProp.get(i).getProp_val_id();
			try{
				Long id = getJdbcTemplate().queryForLong(my_sql);
				questUnitProp.get(i).setProp_val_id(id);
			} catch(Exception e){}
		}
		
		questQuery.getQuestionPropMap().remove(Constants.EXAM_PROP_UNIT);
		questQuery.getQuestionPropMap().put(Constants.EXAM_PROP_UNIT, questUnitProp);
		}
		if (questSource != null) {
			
			for (int i=0; i<questSource.size(); i++) {
				String my_sql = "select prop_val_id from SUB_SYS_PROP_VAL where id=" + questSource.get(i).getProp_val_id();
				try{
					Long id = getJdbcTemplate().queryForLong(my_sql);
					questSource.get(i).setProp_val_id(id);
				} catch(Exception e){}
			}
			
			questQuery.getQuestionPropMap().remove(Constants.EXAM_PROP_SOURCE);
			questQuery.getQuestionPropMap().put(Constants.EXAM_PROP_SOURCE, questSource);
			}
		if (questCognize != null) {
			
			for (int i=0; i<questCognize.size(); i++) {
				String my_sql = "select prop_val_id from SUB_SYS_PROP_VAL where id=" + questCognize.get(i).getProp_val_id();
				try{
					Long id = getJdbcTemplate().queryForLong(my_sql);
					questCognize.get(i).setProp_val_id(id);
				} catch(Exception e){}
			}
			
			questQuery.getQuestionPropMap().remove(Constants.EXAM_PROP_COGNIZE);
			questQuery.getQuestionPropMap().put(Constants.EXAM_PROP_COGNIZE, questCognize);
		}*/
		...
		for(int i=0;i<questList.size();i++){
			...
			if(childQuestList!=null){
				...
				for(int k=0;k<childQuestList.size();k++){
					...
					childQuest.setId(getNextId(Constants.TABLE_EXAM_QUESTION));
	...
	private String setParams(String sql_, ExamQuestionQuery query,
		...
		if (query.getQuestion_label_id() != null) {
			sql.append(" and t.question_label_id ="
					+ query.getQuestion_label_id());
		}
		//区分度
